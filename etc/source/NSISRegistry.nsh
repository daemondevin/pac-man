/* Copy Registry Key
   http://nsis.sourceforge.net/MOVEREGKEY

Description
	Use this function to copy a registry key along with all its values and 
	subkeys to another location in the registry.

	* this does not move.. but copies.
	* the key still remains w/an additional copy to desired location.
...
Usage:
	...
	Section
		${MOVEREGKEY} `SOURCEROOTKEY` `SOURCESUBKEY` `TARGETROOTKEY` `TARGETSUBKEY`
		${MOVEREGKEY} HKLM SOFTWARE\NSIS HKLM SOFTWARE\BACKUP\NSIS
	SectionEnd

Uninstaller:
	!include registry.nsh
	!insertmacro UN.MOVEREGKEY

	Section uninstall
		${UN.MOVEREGKEY} `SOURCEROOTKEY` `SOURCESUBKEY` `TARGETROOTKEY` `TARGETSUBKEY`
		${UN.MOVEREGKEY} HKLM SOFTWARE\NSIS HKLM SOFTWARE\BACKUP\NSIS
	SectionEnd
*/
!ifndef NSISREGISTRY_NSH
!define NSISREGISTRY_NSH
...
${LABEL}:
!macroend
!macro MOVEREGKEY_BASE TYPE
!ifndef ${TYPE}MOVEREGKEY_DEFINE
!define ${TYPE}MOVEREGKEY_DEFINE
...
!macroend
!macro MOVEREGKEY
!insertmacro MOVEREGKEY_BASE ""
!macroend
!macro UN.MOVEREGKEY
!insertmacro MOVEREGKEY_BASE "Un."
!macroend
!macro CALL_MOVEREGKEY_BASE TYPE SOURCEROOTKEY SOURCESUBKEY TARGETROOTKEY TARGETSUBKEY
!ifdef ${TYPE}MOVEREGKEY_DEFINE
    Push $R1
...
!else
!error "Macro ${TYPE}MOVEREGKEY not inserted"
!endif
!macroend
####################################################################################################
!macro CALL_MOVEREGKEY SOURCEROOTKEY SOURCESUBKEY TARGETROOTKEY TARGETSUBKEY
!insertmacro CALL_MOVEREGKEY_BASE "" "${SOURCEROOTKEY}" "${SOURCESUBKEY}" "${TARGETROOTKEY}" "${TARGETSUBKEY}"
!macroend
####################################################################################################
!macro CALL_UN.MOVEREGKEY SOURCEROOTKEY SOURCESUBKEY TARGETROOTKEY TARGETSUBKEY
!insertmacro CALL_MOVEREGKEY_BASE "Un." "${SOURCEROOTKEY}" "${SOURCESUBKEY}" "${TARGETROOTKEY}" "${TARGETSUBKEY}"
!macroend
####################################################################################################
!define MOVEREGKEY "!insertmacro CALL_MOVEREGKEY"
!define UN.MOVEREGKEY "!insertmacro UN.CALL_MOVEREGKEY"
!endif