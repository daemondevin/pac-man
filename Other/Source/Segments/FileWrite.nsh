...
${SegmentPrePrimary}
	...
	${Do}
		IntOp $R0 $R0 + 1
		ClearErrors
		ReadINIStr $0 `${LAUNCHER}` FileWrite$R0 Type
		ReadINIStr $7 `${LAUNCHER}` FileWrite$R0 File
		${IfThen} ${Errors} ${|} ${ExitDo} ${|}
		ExpandEnvStrings $0 $0
		ExpandEnvStrings $7 $7
		${If} $0 == ConfigWrite
			ReadINIStr $2 `${LAUNCHER}` FileWrite$R0 Entry
			ReadINIStr $3 `${LAUNCHER}` FileWrite$R0 Value
			${IfThen} ${Errors} ${|} ${ExitDo} ${|}
			ExpandEnvStrings $2 $2
			ExpandEnvStrings $3 $3
			ClearErrors
			ReadINIStr $4 `${LAUNCHER}` FileWrite$R0 CaseSensitive
			${If} $4 != true
			${AndIf} $4 != false
			${AndIfNot} ${Errors}
			...
			${EndIf}
		${ElseIf} $0 == INI
			ReadINIStr $2 `${LAUNCHER}` FileWrite$R0 Section
			ReadINIStr $3 `${LAUNCHER}` FileWrite$R0 Key
			ReadINIStr $4 `${LAUNCHER}` FileWrite$R0 Value
			${IfThen} ${Errors} ${|} ${ExitDo} ${|}
			ExpandEnvStrings $2 $2
			ExpandEnvStrings $3 $3
...
!ifdef XML_ENABLED
		${ElseIf} $0 == "XML attribute"
			ReadINIStr $R2 `${LAUNCHER}` FileWrite$R0 XPath
			ReadINIStr $R3 `${LAUNCHER}` FileWrite$R0 Attribute
			ReadINIStr $R4 `${LAUNCHER}` FileWrite$R0 Value
			${IfThen} ${Errors} ${|} ${ExitDo} ${|}
		...
		${ElseIf} $0 == "XML text"
			ReadINIStr $R2 `${LAUNCHER}` FileWrite$R0 XPath
			ReadINIStr $R3 `${LAUNCHER}` FileWrite$R0 Value
			${IfThen} ${Errors} ${|} ${ExitDo} ${|}
			ExpandEnvStrings $R2 $R2
			ExpandEnvStrings $R3 $R3
!else
...
!ifdef REPLACE
		${ElseIf} $0 == Replace
			ReadINIStr $2 `${LAUNCHER}` FileWrite$R0 Find
			ReadINIStr $3 `${LAUNCHER}` FileWrite$R0 Replace
			${IfThen} ${Errors} ${|} ${ExitDo} ${|}
			ExpandEnvStrings $2 $2
			ExpandEnvStrings $3 $3
			ClearErrors
...
!ifdef REPLACE
			${ElseIf} $0 == Replace
				ClearErrors
				ReadINIStr $5 `${LAUNCHER}` FileWrite$R0 Encoding
				${If} ${Errors}
					FileOpen $9 $R1 r
					FileReadByte $9 $5
					FileReadByte $9 $6
