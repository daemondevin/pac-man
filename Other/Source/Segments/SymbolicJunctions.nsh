...
Function GetLinkTarget
    ...
    Push $1 ; File handle
    Push $2 ; Buffer for target path
    Push $3 ; Buffer size
    Push $4 ; Bytes returned
    Push $5 ; Link type result
    Push $6 ; Temp variable
    
    ; Initialize variables
    StrCpy $2 ""
    StrCpy $5 ""
    
    ; First, try to get file attributes to check if it's a reparse point
    System::Call "kernel32::GetFileAttributesW(w '$0') i.r6"
    IntOp $6 $6 & 0x400  ; FILE_ATTRIBUTE_REPARSE_POINT
    ${If} $6 != 0
        ; It's a reparse point (symbolic link or junction)
        Call GetReparsePointTarget
        Pop $2
        StrCpy $5 "reparse"
    ${Else}
        ; Check if it's a hardlink by comparing file index
        Call CheckHardLink
        Pop $2
        Pop $5
    ${EndIf}
    ...
    Pop $3
    Pop $1
    Exch $2 ; Return target path
    Exch 
    Pop $0
FunctionEnd
; Function to get reparse point target (symbolic links and junctions)
Function GetReparsePointTarget
    Push $0 ; Input path (from parent function)
    Push $1 ; File handle
    Push $2 ; Buffer
    Push $3 ; Bytes returned
    Push $4 ; Reparse data buffer
    Push $5 ; Target path
    ...
    ${If} $1 = -1  ; INVALID_HANDLE_VALUE
        StrCpy $5 ""
        Goto GetReparsePointTarget_End
    ${EndIf}
    
    ; Allocate buffer for reparse data (16KB should be enough)
    System::Alloc 16384
    Pop $4
    ...
    ${If} $2 = 0
        ...
        StrCpy $5 ""
        Goto GetReparsePointTarget_Cleanup
    ${EndIf}
    
    ; Parse the reparse data structure
    ; Check reparse tag (first 4 bytes)
    System::Call "*$4(i .r2)"
    ${If} $2 = 0xA0000003  ; IO_REPARSE_TAG_MOUNT_POINT (Junction)
        ; Junction point - target is at offset 8 + SubstituteNameOffset
        System::Call "*$4(i, i, &i2 .r2, &i2 .r3)"  ; Get SubstituteNameOffset and SubstituteNameLength
        IntOp $2 $2 + 16  ; Add header size (8) + structure offset (8)
        IntOp $2 $2 + $4  ; Add buffer base address
        System::Call "*$2(&w$3 .r5)"  ; Read the wide string
    ${ElseIf} $2 = 0xA000000C  ; IO_REPARSE_TAG_SYMLINK (Symbolic Link)
        ; Symbolic link - target is at offset 12 + SubstituteNameOffset  
        System::Call "*$4(i, i, i, &i2 .r2, &i2 .r3)"  ; Get SubstituteNameOffset and SubstituteNameLength
        IntOp $2 $2 + 20  ; Add header size (8) + structure offset (12)
        IntOp $2 $2 + $4  ; Add buffer base address
        System::Call "*$2(&w$3 .r5)"  ; Read the wide string
    ${Else}
        StrCpy $5 ""
    ...
    ${If} $5 != ""
        ...
        ${If} $2 > 4
            StrCpy $3 $5 4
            ${If} $3 == "\??\"
                StrCpy $5 $5 "" 4  ; Remove \??\ prefix
            ${EndIf}
        ${EndIf}
    ${EndIf}
    
GetReparsePointTarget_Cleanup:
    System::Free $4
    System::Call "kernel32::CloseHandle(i r1)"
    
GetReparsePointTarget_End:
    Pop $4
    Pop $3
    Pop $2
    Pop $1
    Pop $0
    Push $5  ; Return target path
FunctionEnd
; Function to check if file is a hardlink
Function CheckHardLink
    Push $0 ; Input path (from parent function)
    Push $1 ; File handle
    Push $2 ; File info structure
    Push $3 ; Number of links
    Push $4 ; Result
    Push $5 ; Target path
    ...
    ${If} $1 = -1  ; INVALID_HANDLE_VALUE
        Goto CheckHardLink_End
    ${EndIf}
    
    ; Allocate structure for BY_HANDLE_FILE_INFORMATION
    System::Alloc 52
    Pop $2
    ...
    ${If} $3 != 0
        ; Get number of hard links (at offset 16)
        System::Call "*$2(i, i, i, i, i .r3)"
        ${If} $3 > 1
            ; This file has multiple hard links
            StrCpy $4 "Multiple hardlinks detected (count: $3)"
            StrCpy $5 "hardlink"
        ${EndIf}
    ${EndIf}
        ...
        ${If} $0 == true
            ${If} ${IsAdmin}			
                ...
                ${Do}
                    ...
                    System::Call "kernel32::CreateSymbolicLink(t '$0', t '$1', i $R4) i .6"
                    Pop $6     ; Result
                ...
                ${Do}
                    ...
                    ${If} $6 == "failed"
                        ${If} $3 == "true"
                            MessageBox MB_OK|MB_ICONEXCLAMATION "Error: Cannot create required hard link '$0'Falling back on Copying/Moving files/directories"
                        ${EndIf}
                        ${GetBaseName} $0 $R5
                        ${File::RestorePortable} "$0" "${DATA}\$R5"
                    ${EndIf}
                        ...
                        ${Else}
                            ...
                            ${Switch} $R7
                                ${Case} "Symbolic"
                                    ...
                                    System::Call "kernel32::CreateSymbolicLink(t '$0', t '$R6', i 0) i .6"
                                    Pop $6
                                    ${If} $6 == 0
