...
;
; GlobalCLDefines.nsh
; 	This file handles and declares all the defines for use with PortableApps Compiler in commandline mode.
; 	Be sure to add "/DCLMODE" when using the commandline.
; 	For example:
; 		makensis.exe /DCLMODE ^ 
...
!searchparse /ignorecase /noerrors /file "${APPINFOINI}" `DirectX=` DIRECTX
...
!if ! ${DIRECTX} == ""
	...
	!else if ${DIRECTX} == false
		!undef DIRECTX	 				;= Disable TaskCleanup segment
	!endif
!else
